...
- 通知机器人兼容 TransitHub 的钉钉、企业微信、QQ、飞书和 Telegram 多通道、多实例配置，支持启停、测试和统一扇出。
...
在“全局配置 → 机器人”中可以添加 TransitHub 风格的通知机器人：

- 钉钉和飞书支持 Webhook 签名；企业微信使用群机器人 Webhook；QQ 使用 App ID、App Secret 和用户 OpenID 换取短期访问令牌。
- Telegram 支持 Bot Token、Chat ID 和可选代理；原有 Telegram 配置会自动作为 `legacy-telegram` 机器人兼容读取。
- 机器人凭据使用 `APP_SECRET` 加密保存，接口只返回是否已配置，不返回 Webhook、Token 或 Secret 原文。留空已保存的敏感字段表示保留原值。
- Worker 的余额与倍率变化通知会发送到所有启用的机器人；至少启用一个新机器人时，无需额外打开旧版 Telegram 的两个开关。
...
每轮运行会重新读取全局 Worker 间隔和并发数，并在刷新到期采集站后从目标站同步最新分组，再执行已启用的倍率规则。收到 `SIGINT` 或 `SIGTERM` 后，进程会结束当前周期或等待并安全退出。
