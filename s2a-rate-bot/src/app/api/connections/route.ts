...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ connections: await getRuntimeConnectionService().list() });
...
export async function POST(request: NextRequest) {
  try {
    await requireAuthenticatedRequest(request);
    return NextResponse.json({ connection: await getRuntimeConnectionService().create(await readJsonBody(request)) }, { status: 201 });
  } catch (error) { return connectionError(error); }
}
