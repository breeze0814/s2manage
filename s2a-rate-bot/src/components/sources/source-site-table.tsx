...
export function SourceSiteTable({ sites, pendingId, onRefresh, onEdit, onDelete }: Readonly<{
  ...
  if (sites.length === 0) return <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-600">还没有采集站。</p>;
  return (
    <>
...
function SiteTableHead() {
  return <thead className="bg-slate-50 text-slate-600"><tr><th className="p-3">采集站</th><th className="p-3">类型</th><th className="p-3">状态</th><th className="p-3">账户/余额</th><th className="p-3">最后运行</th><th className="p-3 text-right">操作</th></tr></thead>;
}
...
function SourceRow(props: SourceActions & { site: SourceSiteView; pending: boolean }) {
  ...
  return (
    <tr className="border-t border-slate-200">
      <td className="p-3"><p className="font-medium">{site.name}</p><p className="max-w-64 truncate text-xs text-slate-500">{site.baseUrl}</p></td>
      <td className="p-3">{site.siteType === "newapi" ? "New API" : "Sub2API"}</td>
      <td className="p-3"><Status site={site} /></td>
      <td className="p-3">{site.accountLabel ?? "-"}<p className="text-xs text-slate-500">余额 {site.balance ?? "-"}</p></td>
      <td className="p-3 text-xs text-slate-600">{formatTime(site.lastRunAt)}</td>
...
function SourceCard(props: SourceActions & { site: SourceSiteView; pending: boolean }) {
  ...
  return (
    <article className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3"><div><h3 className="font-medium">{site.name}</h3><p className="break-all text-xs text-slate-500">{site.baseUrl}</p></div><Status site={site} /></div>
...
function IconButton({ label, marker, children, ...button }: Readonly<{ label: string; marker: string; children: React.ReactNode } & React.ButtonHTMLAttributes<HTMLButtonElement>>) {
  return <button type="button" aria-label={label} title={label} {...{ [marker]: true }} {...button} className="flex size-10 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 disabled:opacity-40">{children}</button>;
}
...
function Status({ site }: Readonly<{ site: SourceSiteView }>) {
  ...
  const tone = site.lastStatus === "failed" ? "bg-red-50 text-red-700" : site.enabled ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600";
  return <span title={site.lastError ?? undefined} className={`rounded-full px-2 py-1 text-xs ${tone}`}>{text}</span>;
}

function Info({ label, value }: Readonly<{ label: string; value: string }>) { return <div><dt className="text-xs text-slate-500">{label}</dt><dd>{value}</dd></div>; }
