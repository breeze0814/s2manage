...
export function SourceSiteTable({ sites, selectedSiteId, pendingId, onSelect, onRefresh, onEdit, onDelete }: Readonly<{
  ...
  return <><div role="listbox" aria-label="选择采集站" className="divide-y divide-border">{sites.map((site) => <SourceCard key={site.id} site={site} selected={selectedSiteId === site.id} pending={pendingId === site.id} onSelect={onSelect} onRefresh={onRefresh} onEdit={onEdit} onDelete={setDeleteTarget} />)}</div><ConfirmAlert open={deleteTarget !== null} title="删除采集站" description={`确定删除采集站「${deleteTarget?.name ?? ""}」？此操作无法撤销。`} confirmLabel="确认删除" onOpenChange={(open) => { if (!open) setDeleteTarget(null); }} onConfirm={() => { if (deleteTarget) onDelete(deleteTarget); setDeleteTarget(null); }} /></>;
}

function SourceCard(props: SourceActions & { site: SourceSiteView; selected: boolean; pending: boolean; onSelect: (siteId: number) => void }) {
  const { site } = props;
  return (
    <article role="option" aria-selected={props.selected} tabIndex={0} onClick={() => props.onSelect(site.id)} onKeyDown={(event) => selectWithKeyboard(event, site.id, props.onSelect)} className={`cursor-pointer space-y-3 border-l-2 border-l-border-strong px-3 py-3 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary ${props.selected ? "border-l-primary bg-primary/5" : "hover:bg-surface-muted/55"}`}>
...
function SiteMeta(props: SourceActions & { site: SourceSiteView; pending: boolean }) {
  const { site } = props;
  return <div className="grid gap-2.5 border-t border-border pt-3"><div className="flex min-w-0 flex-wrap gap-1.5"><Tag tone="rate" className="font-mono tabular-nums">充值 ×{site.rechargeRatio}</Tag>{site.useProxy ? <Tag tone="info">使用代理</Tag> : null}</div><ActionButtons {...props} /></div>;
}
...
function ActionButtons({ site, pending, onRefresh, onEdit, onDelete }: SourceActions & { site: SourceSiteView; pending: boolean }) {
  return (
    <div className="flex w-full shrink-0 justify-end gap-1.5" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
...
const SITE_ACTION_CLASS = "inline-flex size-11 items-center justify-center rounded-md border border-border bg-surface text-muted transition-colors hover:border-primary/40 hover:bg-primary/5 hover:text-info disabled:cursor-not-allowed disabled:opacity-40 xl:size-9";
