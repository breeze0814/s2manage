...
function RateToolbar(input: Readonly<{ rateOrder: RateOrder; platform: string; platformOptions: readonly SelectOption[]; status: RateStatus; statusOptions: readonly SelectOption[]; selected: boolean; search: string; onOrderChange: () => void; onPlatformChange: (value: string) => void; onStatusChange: (value: string) => void; onSearch: (value: string) => void; onShowAll: () => void }>) {
  return <div role="group" aria-label="倍率筛选与排序" className="flex flex-wrap items-center gap-2 border-b border-border bg-surface-muted/35 px-4 py-3 lg:px-5"><Label className="relative block min-w-64 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted" aria-hidden="true" /><span className="sr-only">搜索采集倍率</span><Input aria-label="搜索采集倍率" value={input.search} onChange={(event) => input.onSearch(event.target.value)} placeholder="搜索站点、分组或平台" className="pl-9" /></Label><div className="w-36 shrink-0"><Select ariaLabel="平台筛选" value={input.platform} options={input.platformOptions} onValueChange={input.onPlatformChange} /></div><div className="w-40 shrink-0"><Select ariaLabel="映射状态" value={input.status} options={input.statusOptions} onValueChange={input.onStatusChange} /></div><Button type="button" variant="secondary" onClick={input.onOrderChange} className="shrink-0 px-3">{input.rateOrder === "desc" ? <ArrowDownNarrowWide className="size-3.5" /> : <ArrowUpNarrowWide className="size-3.5" />}{rateOrderLabel(input.rateOrder)}</Button><Button type="button" variant="secondary" disabled={!input.selected} onClick={input.onShowAll} className="shrink-0 px-3"><List className="size-3.5" />展示全部</Button></div>;
}
...
function RateCard(props: RateRecordProps) {
  ...
  return <article className="py-4 transition-colors hover:bg-surface-muted/35"><div className="flex justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium" title={rate.groupName}>{rate.groupName}</h3><div className="mt-1 flex flex-wrap items-center gap-1.5"><Tag>#{rate.groupId}</Tag><SiteTag name={siteName} /><ConnectionTags rate={rate} /><Tag><PlatformLabel platform={rate.platform} fallback="未知平台" /></Tag><GroupTypeTag groupType={rate.groupType} /></div></div>{rate.deleted ? <Tag tone="danger">已删除</Tag> : <EffectiveRateValue className="shrink-0 text-lg">×{formatRate(rate.effectiveRate)}</EffectiveRateValue>}</div><dl className="mt-3 grid grid-cols-2 gap-3 border-t border-border pt-3 text-sm"><div><dt className="text-xs text-muted">原始倍率</dt><dd className="mt-1 font-mono tabular-nums text-rate">{formatRate(rate.rawRate)}</dd></div><div><dt className="text-xs text-muted">变化</dt><dd className="mt-1 font-mono text-xs tabular-nums"><DeltaValue rate={rate} /></dd></div><div className="col-span-2"><dt className="text-xs text-muted">最后采集</dt><dd className="mt-1 text-xs leading-5">{formatTime(rate.collectedAt)}</dd></div></dl><div className="mt-3 flex justify-end gap-1"><RateActions {...props} /></div></article>;
}
