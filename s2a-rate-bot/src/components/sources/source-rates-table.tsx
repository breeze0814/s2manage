...
function RateRow({ rate, siteName, pending, onPlatformChange }: Readonly<{ rate: SourceRateView; siteName: string; pending: boolean; onPlatformChange: (rate: SourceRateView, platform: string | null) => void }>) {
  return <tr><td><span className="font-medium">{rate.groupName}</span></td><td><div className="flex items-center gap-2"><span className="font-mono text-xs tabular-nums text-muted">#{rate.groupId}</span><SiteTag name={siteName} /></div></td><td><Tag><PlatformLabel platform={rate.platform} fallback="未知平台" /></Tag></td><td className="font-mono tabular-nums text-rate">{formatRate(rate.rawRate)}</td><td><Tag tone="rate" className="font-mono tabular-nums">×{formatRate(rate.effectiveRate)}</Tag></td><td className="whitespace-nowrap text-xs text-muted">{formatTime(rate.collectedAt)}</td><td><div className="flex justify-end"><PlatformAction rate={rate} pending={pending} onChange={onPlatformChange} /></div></td></tr>;
}
...
function RateCard({ rate, siteName, pending, onPlatformChange }: Readonly<{ rate: SourceRateView; siteName: string; pending: boolean; onPlatformChange: (rate: SourceRateView, platform: string | null) => void }>) {
  return <article className="rounded-xl border border-border bg-surface p-4 transition-colors hover:bg-surface-muted/35"><div className="flex justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium" title={rate.groupName}>{rate.groupName}</h3><div className="mt-1 flex flex-wrap items-center gap-1.5"><Tag>#{rate.groupId}</Tag><SiteTag name={siteName} /><Tag><PlatformLabel platform={rate.platform} fallback="未知平台" /></Tag></div></div><strong className="inline-flex shrink-0 items-center gap-1.5 font-mono text-lg tabular-nums text-rate"><span aria-hidden="true" className="size-1.5 rounded-full bg-rate" />×{formatRate(rate.effectiveRate)}</strong></div><dl className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-3 text-sm"><div><dt className="text-xs text-muted">原始倍率</dt><dd className="mt-1 font-mono tabular-nums text-rate">{formatRate(rate.rawRate)}</dd></div><div><dt className="text-xs text-muted">最后采集</dt><dd className="mt-1 text-xs leading-5">{formatTime(rate.collectedAt)}</dd></div></dl><div className="mt-3 flex justify-end"><PlatformAction rate={rate} pending={pending} onChange={onPlatformChange} /></div></article>;
}
