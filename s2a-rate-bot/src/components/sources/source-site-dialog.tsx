...
export function SourceSiteDialog({ open, site, pending, error, onOpenChange, onSave }: Readonly<{
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-slate-950/50" /><Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[90dvh] w-[min(94vw,620px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl bg-white p-5 shadow-2xl sm:p-6">
        <DialogHeader editing={Boolean(site)} /><SiteForm form={form} update={update} pending={pending} error={error} onSubmit={() => onSave(form)} />
        <Dialog.Close aria-label="关闭" className="absolute right-4 top-4 flex size-10 items-center justify-center rounded-lg hover:bg-slate-100"><X className="size-4" /></Dialog.Close>
...
function DialogHeader({ editing }: Readonly<{ editing: boolean }>) {
  return <div className="mb-5 pr-10"><Dialog.Title className="text-lg font-semibold">{editing ? "编辑采集站" : "添加采集站"}</Dialog.Title><Dialog.Description className="mt-1 text-sm text-slate-600">配置 Sub2API 或 New API 的认证和采集周期。</Dialog.Description></div>;
}
...
function SiteForm({ form, update, pending, error, onSubmit }: Readonly<{
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      ...
      <button type="submit" disabled={pending} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 text-sm font-medium text-white disabled:opacity-50">{pending ? <Loader2 className="size-4 animate-spin" /> : null}{pending ? "保存中..." : "保存采集站"}</button>
    </form>
  );
}
...
function AuthMode({ form, update }: Readonly<{ form: SourceSiteForm; update: UpdateForm }>) {
  return (
    <div className="space-y-3 rounded-lg border border-slate-200 p-4"><div className="flex gap-4 text-sm"><Radio label="账号密码" checked={form.authMode === "password"} onChange={() => update("authMode", "password")} /><Radio label="Token" checked={form.authMode === "manual_token"} onChange={() => update("authMode", "manual_token")} /></div>
...
function Field({ label, hint, children }: Readonly<{ label: string; hint?: string; children: React.ReactNode }>) { return <label className="block space-y-2 text-sm font-medium text-slate-700"><span>{label}</span>{children}{hint ? <span className="block text-xs font-normal text-slate-500">{hint}</span> : null}</label>; }
...
function Toggle({ label, checked, onChange }: Readonly<{ label: string; checked: boolean; onChange: (value: boolean) => void }>) { return <div className="flex min-h-11 items-center justify-between rounded-lg border border-slate-200 px-3"><span className="text-sm font-medium">{label}</span><Switch.Root checked={checked} onCheckedChange={onChange} className="h-6 w-11 rounded-full bg-slate-300 p-0.5 data-[state=checked]:bg-slate-900"><Switch.Thumb className="block size-5 rounded-full bg-white shadow transition-transform data-[state=checked]:translate-x-5" /></Switch.Root></div>; }
