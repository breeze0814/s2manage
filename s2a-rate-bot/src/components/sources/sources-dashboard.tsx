...
export function SourcesDashboard() {
  ...
  const activeSiteId = view.sites.some((site) => site.id === selectedSiteId) ? selectedSiteId : null;
  return (
    <section className="page-stack">
      ...
      <div data-source-split-layout className="grid items-start gap-6 lg:grid-cols-[minmax(320px,0.75fr)_minmax(0,1.65fr)]">
...
function DashboardHeader({ actions }: Readonly<{ actions: React.ReactNode }>) {
  return <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h1 className="page-heading">Rate Sources</h1><p className="page-description">倍率采集</p></div>{actions}</header>;
}

function SourceActions({ bulkPending, onCreate, onRefreshAll }: Readonly<{ bulkPending: boolean; onCreate: () => void; onRefreshAll: () => void }>) {
  return <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end"><Action primary icon={<Plus className="size-3.5" />} label="添加采集站" text="添加站点" onClick={onCreate} /><Action icon={bulkPending ? <Loader2 className="size-3.5 animate-spin" /> : <RefreshCw className="size-3.5" />} label="重新请求全部远端" text="刷新全部" onClick={onRefreshAll} disabled={bulkPending} /></div>;
}
