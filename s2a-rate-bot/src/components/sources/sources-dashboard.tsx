  ...
  return (
    <section className="space-y-5">
      ...
      <div className="space-y-4 rounded-xl border border-border bg-surface p-5 shadow-sm">
        <div><h2 className="font-semibold">采集站</h2><p className="mt-1 text-sm text-muted">管理认证信息和单站远端刷新。</p></div>
...
function DashboardHeader({ sites, enabled, rates }: Readonly<{ sites: number; enabled: number; rates: number }>) {
  return <header><h1 className="text-xl font-semibold tracking-tight">倍率采集</h1><p className="mt-1 text-sm text-muted">管理 Sub2API 与 New API 采集站，并查看最近成功倍率。</p><div className="mt-4 grid grid-cols-3 gap-3"><Metric label="采集站" value={sites} /><Metric label="已启用" value={enabled} /><Metric label="分组倍率" value={rates} /></div></header>;
}
...
function Action({ icon, label, ...button }: Readonly<{ icon: React.ReactNode; label: string } & React.ButtonHTMLAttributes<HTMLButtonElement>>) {
  return <button type="button" {...button} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border-strong bg-surface px-3 text-sm font-medium hover:bg-surface-muted disabled:opacity-50">{icon}{label}</button>;
}
