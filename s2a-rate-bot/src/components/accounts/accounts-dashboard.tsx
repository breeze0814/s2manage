  ...
  return <>
    <section className="page-stack">
      <header className="page-header">
        <div><h1 className="page-heading">账号调度</h1><p className="page-description">查看账号状态、倍率绑定与调度可用性</p></div>
      ...
      {view.loading ? <LoadingAccounts /> : <AccountList accounts={view.accounts} groups={view.groups} rates={view.rates} sites={view.sites}
        testPendingIds={view.testPendingIds} bindingPendingId={view.bindingPendingId} schedulePendingId={view.schedulePendingId}
        onBind={view.bindSource} onTest={view.testChannel} onSchedule={requestScheduleChange} />}
    </section>
    <ConfirmAlert open={scheduleTarget !== null} title="禁用账号调度"
      description={`确定禁用账号「${scheduleTarget?.name ?? ""}」的调度？账号仍会保留，可随时重新启用。`}
      confirmLabel="禁用调度" onOpenChange={(open) => { if (!open) setScheduleTarget(null); }} onConfirm={confirmDisable} />
  </>;
}
...
function AccountList(input: AccountListProps) {
  ...
  const actionProps = (account: TargetAccountView): AccountActionProps => ({
    ...
    bindingDisabled: input.bindingPendingId !== null, scheduling: input.schedulePendingId === account.id,
    onBind: input.onBind, onTest: input.onTest, onSchedule: input.onSchedule,
  });
  return <div className="space-y-3">
    <div className="grid gap-3 lg:grid-cols-2 xl:hidden">{input.accounts.map((account) => <AccountCard key={account.id} {...actionProps(account)} />)}</div>
    <div className="table-shell desktop-table-viewport hidden xl:block"><table className="data-table data-table-sticky min-w-[1180px]"><AccountHead /><tbody>{input.accounts.map((account) => <AccountRow key={account.id} {...actionProps(account)} />)}</tbody></table></div>
  </div>;
}

function AccountHead() {
  return <thead><tr><th className="text-right">ID</th><th>账号</th><th>平台</th><th>账号状态</th><th>调度</th><th>目标分组</th><th className="text-right">优先级 / 倍率</th><th>倍率采集绑定</th><th>测试状态</th><th className="sticky-action-header">操作</th></tr></thead>;
}

function AccountRow(props: AccountActionProps) {
  const { account } = props;
  return <tr><td className="text-right font-mono text-sm tabular-nums text-muted">#{account.id}</td><td><p className="font-medium">{account.name}</p></td><td><Tag><PlatformLabel platform={account.platform} /></Tag></td><td><StatusBadge status={account.status} /></td><td><ScheduleBadge schedulable={account.schedulable} /></td><td><GroupIds ids={account.groupIds} groups={props.groups} /></td><td className="text-right"><AccountMetrics account={account} /></td><td><AccountBindingCell {...props} /></td><td><TestStatus test={account.lastTest} pending={props.testing} /></td><td className="sticky-action-cell"><AccountActions {...props} /></td></tr>;
}

function AccountCard(props: AccountActionProps) {
  const { account } = props;
  return <article className="panel space-y-4 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate font-medium" title={account.name}>{account.name}</h2><div className="mt-1 flex flex-wrap items-center gap-1.5"><Tag><PlatformLabel platform={account.platform} /></Tag><Tag>#{account.id}</Tag></div></div><div className="flex shrink-0 flex-col items-end gap-1.5"><StatusBadge status={account.status} /><ScheduleBadge schedulable={account.schedulable} /></div></div><dl className="grid gap-4 text-sm sm:grid-cols-2"><Info label="目标分组"><GroupIds ids={account.groupIds} groups={props.groups} /></Info><Info label="优先级 / 倍率"><AccountMetrics account={account} /></Info><Info label="倍率采集绑定" wide><AccountBindingCell {...props} /></Info><Info label="测试状态"><TestStatus test={account.lastTest} pending={props.testing} /></Info></dl><div className="flex items-center justify-between border-t border-border pt-3"><span className="text-sm text-muted">操作</span><AccountActions {...props} /></div></article>;
}

function AccountBindingCell(props: AccountActionProps) {
  return <div className="min-w-40 max-w-56">
...
function ScheduleButton({ account, testing, scheduling, onSchedule }: AccountActionProps) {
  ...
  return <button type="button" aria-label={label} title={account.schedulable ? "禁用调度" : "启用调度"}
    aria-pressed={account.schedulable} disabled={testing || scheduling} onClick={() => onSchedule(account)} className={`icon-button ${account.schedulable ? "text-success" : "text-muted"}`}>
...
function ScheduleBadge({ schedulable }: Readonly<{ schedulable: boolean }>) { return <Tag tone={schedulable ? "success" : "warning"}>{schedulable ? "参与调度" : "已暂停"}</Tag>; }
...
function LoadingAccounts() { return <div className="loading-state"><Loader2 className="size-4 animate-spin" />正在读取本地账号快照...</div>; }
...
type AccountActionProps = { readonly account: TargetAccountView; readonly groups: ReadonlyMap<number, AccountGroupOption>; readonly rates: readonly AccountSourceRate[]; readonly sites: readonly AccountSourceSite[]; readonly testing: boolean; readonly bindingPending: boolean; readonly bindingDisabled: boolean; readonly scheduling: boolean; readonly onBind: (account: TargetAccountView, binding: AccountSourceBinding | null) => Promise<boolean>; readonly onTest: (account: TargetAccountView) => void; readonly onSchedule: (account: TargetAccountView) => void };
type AccountListProps = Readonly<{ accounts: readonly TargetAccountView[]; groups: readonly AccountGroupOption[]; rates: readonly AccountSourceRate[]; sites: readonly AccountSourceSite[]; testPendingIds: readonly number[]; bindingPendingId: number | null; schedulePendingId: number | null; onBind: (account: TargetAccountView, binding: AccountSourceBinding | null) => Promise<boolean>; onTest: (account: TargetAccountView) => void; onSchedule: (account: TargetAccountView) => void }>;
