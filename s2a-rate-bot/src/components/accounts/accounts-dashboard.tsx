...
function AccountList(input: AccountListProps) {
  ...
  return <div className="space-y-3">
    ...
    <div className="hidden overflow-x-auto md:block"><div className="table-shell min-w-[1320px]"><table className="data-table"><AccountHead /><tbody>{input.accounts.map((account) => <AccountRow key={account.id} {...actionProps(account)} />)}</tbody></table></div></div>
  </div>;
}

function AccountHead() {
  return <thead><tr><th>ID</th><th>账号</th><th>平台</th><th>账号状态</th><th>调度</th><th>目标分组</th><th>优先级 / 倍率</th><th>倍率采集绑定</th><th>测试状态</th><th className="text-right">操作</th></tr></thead>;
}
...
function ScheduleButton({ account, testing, scheduling, onSchedule }: AccountActionProps) {
  ...
  return <button type="button" aria-label={label} title={account.schedulable ? "禁用调度" : "启用调度"}
    aria-pressed={account.schedulable} disabled={testing || scheduling} onClick={() => onSchedule(account)} className={`icon-button ${account.schedulable ? "text-emerald-700 dark:text-emerald-400" : "text-muted"}`}>
