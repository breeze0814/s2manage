...
import { changeLotteryDrawMode, type LotteryFormDraft } from "./lottery-form-model";
...
export function LotteryActivityFields(props: DraftProps) {
  ...
  return <LotteryFormSection id="lottery-activity-heading" icon={CalendarRange} title="活动信息" description="填写活动内容，并选择即时或定时开奖。">
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
      ...
      <DrawModeField value={draft.drawMode} onChange={(drawMode) => props.onChange(changeLotteryDrawMode(draft, drawMode))} />
    </div>
    <div className="mt-5 border-t border-border pt-5">
      ...
      <div className={`grid gap-4 sm:grid-cols-2 ${draft.drawMode === "scheduled" ? "lg:grid-cols-3" : ""}`}>
        <DateField id="lottery-registration-start" label="活动开始" value={draft.registrationStart} onChange={(registrationStart) => props.onChange({ ...draft, registrationStart })} />
        <DateField id="lottery-registration-end" label="活动结束" value={draft.registrationEnd} onChange={(registrationEnd) => props.onChange({ ...draft, registrationEnd })} />
...
function DrawModeField(props: Readonly<{
  ...
  onChange: (value: LotteryFormDraft["drawMode"]) => void;
}>) {
  return <fieldset><legend className="mb-2 text-sm font-semibold">开奖方式</legend>
    <RadioGroup value={props.value} onValueChange={(value) => props.onChange(value as LotteryFormDraft["drawMode"])} className="grid gap-2">
      <DrawModeOption value="instant" icon={Sparkles} title="即时开奖" description="用户转动轮盘后立即返回结果" />
      <DrawModeOption value="scheduled" icon={CalendarClock} title="定时开奖" description="先报名，到达设定时间后统一开奖" />
    </RadioGroup>
  </fieldset>;
}
