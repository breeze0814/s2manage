...
export function LotteryDashboard() {
  ...
  const load = () => void loadCampaigns(setItems, setLoading, setError);
  useEffect(() => { void loadCampaigns(setItems, setLoading, setError); }, []);
  return (
    <section className="page-stack">
      ...
      {error && !items.length ? <DataLoadError message={`抽奖活动加载失败：${error}`} onRetry={load} pending={loading} className="min-h-36 justify-center" /> : <>
      {error ? <DataLoadError message={`抽奖活动刷新失败：${error}`} onRetry={load} pending={loading} /> : null}
...
function CampaignToolbar(props: Readonly<{
  ...
  return <div className="mt-3 flex flex-col gap-2.5 border-y border-border bg-surface-muted/35 py-2.5 sm:flex-row sm:items-center sm:justify-between">
...
function CampaignList({ items, total, loading, visibilityPendingId, onEdit, onCancel, onVisibilityChange }: Readonly<{ items: readonly LotteryCampaign[]; total: number; loading: boolean; visibilityPendingId: string | null; onEdit: (campaign: LotteryCampaign) => void; onCancel: (id: string) => void; onVisibilityChange: (id: string, visible: boolean) => void }>) {
  ...
  return <div className="grid gap-3 pt-3 lg:grid-cols-2">{items.map((campaign) => <CampaignCard key={campaign.id} campaign={campaign}
...
function CampaignCard({ campaign, visibilityPending, onEdit, onCancel, onVisibilityChange }: Readonly<{ campaign: LotteryCampaign; visibilityPending: boolean; onEdit: () => void; onCancel: () => void; onVisibilityChange: (visible: boolean) => void }>) {
  ...
  return <article className="rounded-lg border border-border bg-surface p-3.5 shadow-sm transition-[border-color,box-shadow] duration-200 hover:border-primary/35 hover:shadow-md">
    ...
    <dl className="mt-3 grid grid-cols-3 divide-x divide-border text-center"><Metric icon={TicketCheck} label="参与" value={campaign.entryCount} /><Metric icon={Gift} label="剩余奖品" value={campaign.prizeInventory.reduce((sum, item) => sum + item.remaining, 0)} /><Metric icon={Trophy} label="中奖" value={campaign.winnerCount} /></dl>
...
function WinnerList({ campaign }: Readonly<{ campaign: LotteryCampaign }>) {
  return <div className="mt-4 border-t border-border pt-3"><p className="text-xs font-semibold text-muted">中奖结果与兑换码</p>
    <ul className="mt-2 space-y-2 text-sm">{campaign.winners.map((winner) => <li key={winner.id} className="rounded-lg bg-surface-muted px-3 py-2">
      ...
      <div className="mt-1 flex flex-wrap items-center justify-between gap-2 text-xs text-muted"><span>{winner.prizeType ? PRIZE_TYPE_LABELS[winner.prizeType] : "-"} · 额度 {winner.prizeValue ?? "-"}</span><span className="flex items-center gap-1"><code className="select-all font-mono text-foreground">{winner.redemptionCode ?? rewardStatusLabel(winner.rewardStatus)}</code>{winner.redemptionCode ? <Button type="button" variant="ghost" size="icon-sm" aria-label="复制兑换码" title="复制兑换码" onClick={() => void copyCode(winner.redemptionCode!)}><Copy className="size-3.5" /></Button> : null}</span></div>
    </li>)}</ul>
  </div>;
}

function Metric({ icon: Icon, label, value }: Readonly<{ icon: typeof Gift; label: string; value: number }>) { return <div className="min-w-0 px-2 py-1"><Icon className="mx-auto size-4 text-muted" /><dd className="mt-1 font-semibold tabular-nums">{value}</dd><dt className="truncate text-xs text-muted">{label}</dt></div>; }
...
function statusLabel(campaign: LotteryCampaign) { if (campaign.status === "open") return campaign.drawMode === "instant" ? "抽奖中" : "报名中"; return { scheduled: "待开始", closed: "报名已结束", drawing: "开奖处理中", drawn: "已开奖", exhausted: "奖品已抽完", cancelled: "已取消" }[campaign.status]; }
function rewardStatusLabel(status: NonNullable<LotteryCampaign["currentEntry"]>["rewardStatus"]) { return status === "retryable_failed" ? "发放失败，等待重试" : "奖励发放中"; }
function formatDate(value: string) { return new Intl.DateTimeFormat("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }).format(new Date(value)); }

async function loadCampaigns(setItems: (items: LotteryCampaign[]) => void, setLoading: (value: boolean) => void, setError: (value: string) => void) { setLoading(true); setError(""); try { setItems((await requestJson<{ items: LotteryCampaign[] }>("/api/lottery/campaigns")).items); } catch (error) { const message = error instanceof Error ? error.message : String(error); setError(message); toast.error(message); } finally { setLoading(false); } }
