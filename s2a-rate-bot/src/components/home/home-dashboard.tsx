...
import { Activity, CheckCircle2, Database, Layers3, Loader2, RefreshCw, WalletCards } from "lucide-react";
...
function Header({ loading, onRefresh }: Readonly<{ loading: boolean; onRefresh: () => void }>) { return <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-emerald-700 dark:text-emerald-400"><CheckCircle2 className="size-3.5" />系统数据总览</div><h1 className="page-heading">System Overview</h1><p className="page-description">系统概览</p><p className="mt-2 text-sm text-muted">汇总采集站、目标分组与最近倍率变化</p></div><button type="button" onClick={onRefresh} disabled={loading} className="secondary-button self-start sm:self-auto">{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}刷新数据</button></header>; }
...
function DashboardContent({ data }: Readonly<{ data: DashboardData }>) {
  ...
  return <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={<Database />} label="采集站" value={data.sites.length} detail={`启用 ${enabledSites}`} tone="blue" /><Metric icon={<WalletCards />} label="采集站总余额" value={formatBalance(totalBalance)} valueClassName="text-balance-value" detail={`${data.sites.filter((site) => site.balance !== null).length} 个站点已采集`} tone="emerald" /><Metric icon={<Layers3 />} label="目标分组" value={data.groups.length} detail={`启用规则 ${enabledRules}`} tone="violet" /><Metric icon={<Activity />} label="倍率快照" value={data.rates.length} detail={failures ? `${failures} 个站点异常` : "全部站点正常"} tone={failures ? "red" : "amber"} /></div><div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.65fr)]"><RateChangePanel changes={data.changes} /><div className="grid gap-4"><WorkerOverview run={data.run} /><SiteBalances sites={data.sites} /></div></div></>;
}

function Metric({ icon, label, value, valueClassName = "", detail, tone }: Readonly<{ icon: React.ReactNode; label: string; value: string | number; valueClassName?: string; detail: string; tone: MetricTone }>) { const style = METRIC_TONES[tone]; return <article className="group relative overflow-hidden rounded-xl border border-border bg-surface p-4 shadow-sm transition-colors hover:border-primary/30"><span className={`absolute inset-x-0 top-0 h-0.5 ${style.line}`} /><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-muted">{label}</p><p className={`mt-2 font-mono text-2xl font-semibold tracking-tight tabular-nums ${valueClassName}`}>{value}</p></div><span className={`flex size-9 items-center justify-center rounded-lg [&>svg]:size-4 ${style.icon}`}>{icon}</span></div><p className={`mt-3 text-xs font-medium ${style.detail}`}>{detail}</p></article>; }
...
function siteStatusClass(site: Site) { if (site.lastStatus === "failed") return "bg-red-500"; return site.enabled ? "bg-emerald-500" : "bg-stone-400"; }
...
const METRIC_TONES = {
  blue: { line: "bg-blue-500", icon: "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300", detail: "text-blue-700 dark:text-blue-400" },
  emerald: { line: "bg-emerald-500", icon: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300", detail: "text-emerald-700 dark:text-emerald-400" },
  violet: { line: "bg-violet-500", icon: "bg-violet-50 text-violet-700 dark:bg-violet-950 dark:text-violet-300", detail: "text-violet-700 dark:text-violet-400" },
  amber: { line: "bg-amber-500", icon: "bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300", detail: "text-amber-700 dark:text-amber-400" },
  red: { line: "bg-red-500", icon: "bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300", detail: "text-red-700 dark:text-red-400" },
} as const;
