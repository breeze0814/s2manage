...
function Header({ loading, onRefresh }: Readonly<{ loading: boolean; onRefresh: () => void }>) { return <header className="flex items-start justify-between gap-4"><div><h1 className="page-heading">System Logs</h1><p className="page-description">系统日志</p></div><button type="button" aria-label="刷新业务日志" title="刷新业务日志" onClick={onRefresh} disabled={loading} className="icon-button">{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}</button></header>; }
