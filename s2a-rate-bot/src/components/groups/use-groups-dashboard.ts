...
export function useGroupsDashboard() {
  ...
  useEffect(() => { void loadAll({ setGroups, setSites, setRates, setLoading }); }, []);
  return {
    groups, sites, rates, loading, pending,
    refresh: () => void refreshGroups({ setGroups, setLoading }),
...
async function refreshGroups(input: Pick<LoadActions, "setGroups" | "setLoading">) {
...
type LoadActions = { setGroups: SetGroups; setSites: React.Dispatch<React.SetStateAction<SourceSiteOption[]>>; setRates: React.Dispatch<React.SetStateAction<SourceRateOption[]>>; setLoading: (value: boolean) => void };
