    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm" aria-label="分组倍率列表">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left text-sm">
          <TableHead />
          <tbody>{groups.map((group) => <GroupRow key={group.id} group={group} sites={sites} rates={rates} rateMap={rateMap} pending={pending} onRefresh={onRefresh} onSave={onSave} onApply={onApply} />)}</tbody>
        </table>
      </div>
    </section>
  );
...
function GroupRow(props: Readonly<{ group: TargetGroupView; sites: readonly SourceSiteOption[]; rates: readonly SourceRateOption[]; rateMap: ReadonlyMap<string, SourceRateOption>; pending: string; onRefresh: GroupRuleTableProps["onRefresh"]; onSave: GroupRuleTableProps["onSave"]; onApply: GroupRuleTableProps["onApply"] }>) {
  const { group, sites, rates, rateMap, pending, onRefresh, onSave, onApply } = props;
