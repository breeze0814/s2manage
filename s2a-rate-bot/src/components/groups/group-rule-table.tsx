...
import { Calculator, Loader2, Play, RefreshCw } from "lucide-react";
...
export function GroupRuleTable(props: GroupRuleTableProps) {
  const { groups, sites, rates, pending, onRefresh, onSave, onPreview, onApply } = props;
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm" aria-label="分组倍率列表">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left text-sm">
          <TableHead />
          <tbody>{groups.map((group) => <GroupRow key={group.id} group={group} sites={sites} rates={rates} rateMap={rateMap} pending={pending} onRefresh={onRefresh} onSave={onSave} onPreview={onPreview} onApply={onApply} />)}</tbody>
        </table>
      </div>
    </section>
  );
...
function GroupRow(props: Readonly<{ group: TargetGroupView; sites: readonly SourceSiteOption[]; rates: readonly SourceRateOption[]; rateMap: ReadonlyMap<string, SourceRateOption>; pending: string; onRefresh: GroupRuleTableProps["onRefresh"]; onSave: GroupRuleTableProps["onSave"]; onPreview: GroupRuleTableProps["onPreview"]; onApply: GroupRuleTableProps["onApply"] }>) {
  const { group, sites, rates, rateMap, pending, onRefresh, onSave, onPreview, onApply } = props;
  const busy = pending.endsWith(`:${group.id}`);
  return <tr className="border-t border-border transition-colors hover:bg-surface-muted/45"><td className="px-4 py-3.5 sm:px-5"><GroupCell group={group} /></td><td className="px-4 py-3.5"><StatusBadge enabled={group.rule.enabled} /></td><td className="px-4 py-3.5"><BindingCell group={group} rateMap={rateMap} /></td><td className="px-4 py-3.5"><RuleCell group={group} /></td><td className="whitespace-nowrap px-4 py-3.5 text-xs text-muted">{formatAppliedAt(group.rule.lastAppliedAt)}</td><td className="px-4 py-3.5 sm:px-5"><div className="flex justify-end gap-2"><IconAction label="刷新此分组" pending={pending === `refresh:${group.id}`} disabled={busy} onClick={() => onRefresh(group.id)} icon={<RefreshCw className="size-4" />} /><GroupRuleDialog group={group} sites={sites} rates={rates} pending={pending} onSave={onSave} /><IconAction label="计算预览" pending={pending === `preview:${group.id}`} disabled={busy || !group.rule.enabled} onClick={() => onPreview(group.id)} icon={<Calculator className="size-4" />} /><IconAction label="立即应用" primary pending={pending === `apply:${group.id}`} disabled={busy || !group.rule.enabled} onClick={() => onApply(group.id)} icon={<Play className="size-4" />} /></div></td></tr>;
}

function GroupCell({ group }: Readonly<{ group: TargetGroupView }>) {
  return <div><div className="flex items-center gap-2"><p className="max-w-36 truncate font-medium" title={group.name}>{group.name}</p><span className="text-xs tabular-nums text-muted">#{group.id}</span></div><div className="mt-1.5 flex flex-wrap items-center gap-1.5"><Tag><PlatformLabel platform={group.platform} fallback="未知平台" /></Tag><p className="inline-flex items-center gap-1.5 font-mono text-base font-semibold tabular-nums text-foreground"><span aria-hidden="true" className="size-1.5 rounded-full bg-primary" />{formatRate(group.rate_multiplier)}</p></div></div>;
}

function BindingCell({ group, rateMap }: Readonly<{ group: TargetGroupView; rateMap: ReadonlyMap<string, SourceRateOption> }>) {
  if (group.bindings.length === 0) return <span className="text-sm text-muted">尚未绑定</span>;
  return <div className="flex max-w-md flex-col items-start gap-1.5">{group.bindings.map((binding) => { const rate = rateMap.get(`${binding.sourceSiteId}:${binding.sourceGroupId}`); return <Tag key={`${binding.sourceSiteId}:${binding.sourceGroupId}`} title={rate?.groupName ?? binding.sourceGroupId} className="w-full justify-between"><span className="min-w-0 truncate text-foreground">{rate?.groupName ?? binding.sourceGroupId}</span><span className="shrink-0 font-mono font-semibold tabular-nums text-foreground">×{rate?.effectiveRate ?? "-"}</span></Tag>; })}</div>;
}

function RuleCell({ group }: Readonly<{ group: TargetGroupView }>) {
  const parameters = group.rule.parameters;
  return <div><Tag tone="primary">{RULE_LABELS[group.rule.ruleType]}</Tag><p className="mt-1.5 font-mono text-xs tabular-nums text-muted">偏移 {parameters.offset}</p><p className="mt-1 font-mono text-xs tabular-nums text-muted">固定下限 {parameters.minimum}</p>{group.rule.ruleType === "avg_formula" ? <p className="mt-1 max-w-48 truncate font-mono text-xs" title={parameters.formula}>{parameters.formula}</p> : null}{group.rule.lastError ? <p className="mt-1 max-w-48 truncate text-xs text-red-600 dark:text-red-400" title={group.rule.lastError}>{group.rule.lastError}</p> : null}</div>;
}
...
function IconAction({ label, icon, pending, primary = false, ...button }: Readonly<{ label: string; icon: React.ReactNode; pending: boolean; primary?: boolean } & React.ButtonHTMLAttributes<HTMLButtonElement>>) {
  return <button type="button" aria-label={label} title={label} {...button} className={primary ? "icon-button-primary" : "icon-button"}>{pending ? <Loader2 className="size-4 animate-spin" /> : icon}<span className="sr-only">{label}</span></button>;
}
