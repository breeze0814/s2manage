...
function ConnectionToolbar({ search, status, connections, onSearch, onStatus }: Readonly<{ search: string; status: string; connections: readonly ConnectionView[]; onSearch: (value: string) => void; onStatus: (value: string) => void }>) {
  ...
  return <div className="flex flex-wrap gap-2 border-b border-border bg-surface-muted/35 px-4 py-3"><Label className="relative min-w-64 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><span className="sr-only">搜索真实连接</span><Input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="搜索站点、分组或目标账号" className="pl-9" /></Label><div className="w-40"><Select ariaLabel="连接状态筛选" value={status} options={options} onValueChange={onStatus} /></div></div>;
}
...
function ConnectionCard({ connection, monitor, actions }: Readonly<{ connection: ConnectionView; monitor: HealthMonitor | null; actions: Actions }>) {
  return <article className="rounded-lg border border-border bg-surface p-4"><div className="flex items-start justify-between gap-3"><SourceIdentity connection={connection} /><ConnectionStatusTag connection={connection} /></div><div className="mt-3 flex flex-wrap gap-1.5"><Tag>{groupTypeLabel(connection.groupType)}</Tag><Tag tone={connection.pricingMappingEnabled ? "success" : "neutral"}>{connection.pricingMappingEnabled ? "已用于调价" : "未用于调价"}</Tag><HealthStatus monitor={monitor} /></div><dl className="mt-4 grid gap-3 border-t border-border pt-3 text-sm sm:grid-cols-2"><Info label="目标转发账号"><TargetAccount connection={connection} /></Info><Info label="目标分组"><TargetGroups names={connection.targetGroupNames} /></Info><Info label="最后探测"><ProbeDetail monitor={monitor} /></Info></dl><div className="mt-4 flex justify-end border-t border-border pt-3"><ConnectionActions connection={connection} monitor={monitor} actions={actions} /></div></article>;
}
