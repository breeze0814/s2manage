...
export function EmbedHeader({ eyebrow, title, description }: Readonly<{ eyebrow: string; title: string; description: string }>) {
  return <header className="border-b border-border bg-surface/95 px-4 py-5 shadow-sm"><div className="mx-auto max-w-6xl"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-strong">{eyebrow}</p><h1 className="mt-1 text-xl font-semibold sm:text-2xl">{title}</h1><p className="mt-1 text-sm leading-6 text-muted">{description}</p></div></header>;
}
