...
import type { AppStorage, SourceOverviewInput, TargetAccountSnapshot, TargetGroupSnapshot } from "../storage/app-config.ts";
...
function handleAppConfig(input: {
  ...
  return requireStorage(input.storage).getAppConfig().then((config) => {
    sendJson(input.response, 200, config);
    return true;
  });
}
...
async function handleTargetSettings(input: RouteInput) {
  ...
  sendJson(input.response, 200, { target: await requireStorage(input.storage).saveTargetSettings(settings) });
  return true;
}
