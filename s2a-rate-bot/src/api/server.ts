...
import { errorMessage, readJsonBody, sendError, sendJson } from "./http.ts";
import {
...
async function handleTargetGroups(
  ...
  const input = targetConfigSchema.parse(await readJsonBody(request));
...
async function handleTargetGroupRate(
  ...
  const input = updateGroupRateSchema.parse(await readJsonBody(request));
  const client = new Sub2ApiAdminTarget(input.baseUrl, input.adminApiKey);
...
async function handleTargetAccountSchedulable(
  ...
  const input = updateAccountSchedulableSchema.parse(await readJsonBody(request));
  const client = new Sub2ApiAdminTarget(input.baseUrl, input.adminApiKey);
...
async function handleSourceRates(
  ...
  const input = sourceRatesSchema.parse(await readJsonBody(request));
...
function sourceAuth(input: z.infer<typeof sourceRatesSchema>) {
  if (input.authMode === "password") {
    if (!input.username || !input.password) throw new Error("账号密码认证需要 username 和 password");
    return {
  ...
  if (!input.accessToken && !input.rtToken) throw new Error("手动 Token 认证需要 accessToken 或 rtToken");
  return {
...
async function handleApiRoute(
  ...
  else if (pathname === "/api/source/rates") await handleSourceRates(request, response, proxyUrl);
  ...
  return async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      if (url.pathname === "/api/status") {
        sendJson(response, 200, { services: buildStatus({ databaseUrl: config.databaseUrl }) });
        return;
      }
      ...
      sendError(response, error instanceof z.ZodError ? 400 : 502, error);
    }
  };
}
