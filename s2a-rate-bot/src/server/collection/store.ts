...
import type { CollectionOverview, CollectionSiteStored } from "./types.ts";
...
function createSite(database: DatabaseSync, site: SiteWrite) {
  ...
  const result = database.prepare(`${siteInsertSql()} VALUES (
    :name, :siteType, :baseUrl, :authMode, :username, :passwordEnc, :accessTokenEnc,
...
function updateSite(database: DatabaseSync, id: number, site: SiteWrite) {
  const result = database.prepare(`UPDATE collection_sites SET name=:name, site_type=:siteType,
    base_url=:baseUrl, auth_mode=:authMode, username=:username, password_enc=:passwordEnc,
...
function siteInsertSql() {
  return `INSERT INTO collection_sites (
    name, site_type, base_url, auth_mode, username, password_enc, access_token_enc,
...
function insertRun(database: DatabaseSync, run: Record<string, string | number | null>) {
  database.prepare(`INSERT INTO collection_runs (site_id, status, error, group_count, started_at, finished_at)
...
function readRates(database: DatabaseSync, siteId?: number): SourceRateSnapshot[] {
  const sql = `SELECT * FROM collection_group_rates${siteId ? " WHERE site_id = ?" : ""} ORDER BY site_id, group_id`;
  const rows = (siteId ? database.prepare(sql).all(siteId) : database.prepare(sql).all()) as Record<string, unknown>[];
  return rows.map((row) => ({ sourceSiteId: Number(row.site_id), groupId: String(row.group_id), groupName: String(row.group_name), platform: row.platform ? String(row.platform) : undefined, rawRate: row.raw_rate === null ? null : Number(row.raw_rate), effectiveRate: Number(row.effective_rate), collectedAt: new Date(String(row.collected_at)) }));
}
