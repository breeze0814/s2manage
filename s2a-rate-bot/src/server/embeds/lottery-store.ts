...
export type StoredCampaign = Omit<LotteryCampaign,
  "entryCount" | "winnerCount" | "currentEntry" | "winners" | "prizeInventory">;
...
export function createSqliteLotteryStore(databaseUrl: string) {
  ...
  return {
    ...
    withdraw: (campaignId: string, userId: string, at: string) => withdraw(database, { campaignId, userId, timestamp: at }),
...
function readEntry(database: DatabaseSync, campaignId: string, userId: string) {
  const row = database.prepare(`SELECT * FROM embed_lottery_entries
    WHERE campaign_id = ? AND sub2api_user_id = ?`).get(campaignId, userId) as EntryRow | undefined;
  return row ? mapEntry(row) : null;
}

function insertEntry(database: DatabaseSync, entry: LotteryEntry) {
  database.prepare(`INSERT INTO embed_lottery_entries
    (id, campaign_id, sub2api_user_id, masked_email, status, prize_id, prize_name, prize_type,
      prize_value, redemption_code, reward_code_id, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(...entryValues(entry));
  return requiredEntry(database, entry.campaignId, entry.sub2apiUserId);
}

function enterScheduled(database: DatabaseSync, entry: LotteryEntry) {
  const current = readEntry(database, entry.campaignId, entry.sub2apiUserId);
  ...
  const result = database.prepare(`UPDATE embed_lottery_entries SET status = 'entered', masked_email = ?,
    ...
    WHERE campaign_id = ? AND sub2api_user_id = ? AND status = 'withdrawn'`)
    .run(entry.maskedEmail, entry.updatedAt, entry.campaignId, entry.sub2apiUserId);
...
function recordInstantLoss(database: DatabaseSync, entry: LotteryEntry) {
  ...
  transaction(database, () => {
    ...
    if (readEntry(database, entry.campaignId, entry.sub2apiUserId)) {
...
function reserveInstant(database: DatabaseSync, entry: LotteryEntry, prize: LotteryPrize): LotteryEntry | null {
  ...
  transaction(database, () => {
    ...
    reserved = requiredEntry(database, entry.campaignId, entry.sub2apiUserId);
  });
  return reserved;
}
...
function withdraw(database: DatabaseSync, withdrawal: Readonly<{
  campaignId: string; userId: string; timestamp: string;
}>) {
  const result = database.prepare(`UPDATE embed_lottery_entries SET status = 'withdrawn', updated_at = ?
    WHERE campaign_id = ? AND sub2api_user_id = ? AND status = 'entered'`)
    .run(withdrawal.timestamp, withdrawal.campaignId, withdrawal.userId);
  return result.changes === 1 ? requiredEntry(database, withdrawal.campaignId, withdrawal.userId) : null;
}
...
function mapEntry(row: EntryRow): LotteryEntry {
  return {
    id: row.id, campaignId: row.campaign_id, sub2apiUserId: row.sub2api_user_id,
...
function entryValues(entry: LotteryEntry) {
  return [entry.id, entry.campaignId, entry.sub2apiUserId, entry.maskedEmail, entry.status,
...
function requiredEntry(database: DatabaseSync, campaignId: string, userId: string) {
  const entry = readEntry(database, campaignId, userId);
