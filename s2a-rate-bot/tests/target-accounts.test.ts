...
function assertAccountsDashboard() {
  ...
  assert.match(dashboard, /<TableHead className="text-right">ID<\/TableHead>\s*<TableHead>账号<\/TableHead>/);
  assert.match(dashboard, /<TableCell className="text-right font-mono text-sm tabular-nums text-muted">#\{account\.id\}<\/TableCell>/);
  assert.match(dashboard, /sticky-action-header/);
  assert.match(dashboard, /sticky-action-cell/);
  assert.match(dashboard, /刷新账号/);
  assert.match(dashboard, /variant="secondary"[\s\S]*className="shrink-0"/);
  ...
  assert.match(bindingDialog, /from "\.\.\/ui\/dialog"/);
  ...
  assert.match(bindingDialog, /<DialogTrigger asChild>\s*<Button/);
